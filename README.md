...
O resultado segue o formato `artifacts/PauseCut-<versão>.0-Windows-x64.msix`, acompanhado do SHA-256. Para uma nova revisão enviada à Store, incremente a versão do aplicativo; o quarto número do pacote precisa permanecer zero. Para submissões MSIX, o Partner Center substitui a assinatura depois que o aplicativo passa pela certificação. O MSIX usa a identidade reservada, inclui os recursos visuais, os executáveis autocontidos e os avisos/licenças do FFmpeg.
