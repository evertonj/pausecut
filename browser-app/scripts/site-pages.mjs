...
export function validateConfig(config) {
  ...
  if(config.desktopDownload?.url) {
    ...
    if(download.protocol!=='https:' || download.origin!==url.origin || !download.pathname.endsWith('.exe')) throw new Error('Desktop download must be a same-origin HTTPS installer');
    const checksum = new URL(config.desktopDownload.sha256Url);
    if(checksum.origin!==url.origin || checksum.href!==download.href+'.sha256') throw new Error('Desktop checksum URL must match the installer URL');
  }
...
export function desktopDownload(config) {
  ...
  const action=available
    ? `<a class="desktop-cta" href="${escapeHtml(download.url)}" download>Baixar instalador do PauseCut <span>↓</span></a>`
    : `<span class="desktop-cta disabled" aria-disabled="true">Pacote em preparação</span>`;
  return `<section id="download-desktop" class="desktop-download" aria-labelledby="desktop-title"><div class="desktop-copy"><span class="desktop-kicker">MAIS VELOCIDADE, MESMA PRIVACIDADE</span><h2 id="desktop-title">Use toda a potência do seu PC.</h2><p>A versão desktop executa o FFmpeg nativo, detecta aceleração NVIDIA, Intel ou AMD e volta automaticamente para todos os processadores da CPU quando necessário. O vídeo continua no seu computador.</p><ul><li>Windows 10/11 de 64 bits</li><li>Instalador com atalhos e desinstalação</li><li>Sem limite fixo de tamanho do vídeo</li><li>Sem instalar .NET ou FFmpeg</li></ul></div><div class="desktop-action"><span class="desktop-badge">WINDOWS</span>${action}<strong>${details}</strong><small>Execute o instalador e abra o PauseCut pelo Menu Iniciar. Como o pacote ainda não possui assinatura de código, o Windows pode mostrar “Editor desconhecido”; confira o SHA-256 antes de continuar.</small>${available&&download.sha256Url?`<a class="checksum-link" href="${escapeHtml(download.sha256Url)}">Ver SHA-256</a>`:''}</div></section>`;
}
